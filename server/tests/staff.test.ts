...
async function update(id:string,role='USER',active=true,version=1){return db.query('select update_staff($1,$2,$3,$4,$5)',[id,'Staff',role,active,version]);}
test('USER cannot change roles and final active ADMIN cannot be removed',async()=>{
 await expect(update(user,'ADMIN')).rejects.toThrow();await asUser(admin);
 await expect(update(admin,'USER')).rejects.toThrow('administrateur');
 await expect(update(admin,'ADMIN',false)).rejects.toThrow('administrateur');
});
test('role changes revoke sessions and stale updates fail',async()=>{
 await update(user,'ADMIN');
 await expect(update(user,'USER')).rejects.toThrow('modifié');
 await db.exec('reset role');
 expect((await db.query('select * from auth.sessions where user_id=$1',[user])).rows).toHaveLength(0);
 await asUser(user);await expect(update(admin)).rejects.toThrow();await asUser(admin);
 await update(user,'USER',false,2);
 expect((await db.query("select * from activity_logs where entity_type='profiles'")).rows).toHaveLength(2);
});
