...
export async function PUT(request: Request, context: Context) {
  return handle(async () => {
    ...
    const existing = await identity.db.from('products').select('id,image_path').eq('id',id).maybeSingle();
    databaseError(existing.error);
    if (!existing.data) throw new HttpError(404,'Produit introuvable.');
    const bytes = await sanitizeServiceImage(request);
    const path = `${id}/${randomUUID()}.jpg`;
    const storage = privilegedDatabase().storage.from('product-images');
    ...
    const linked = await identity.db.rpc('set_product_image',{product:id,object_path:path});
