...
create function public.save_product(sku_text text,name_text text,unit_text text,cost integer,threshold numeric,
...
begin
  ...
  if record_id is null then
    insert into public.products(sku,name,unit,cost_centimes,reorder_level,active)
      values(btrim(sku_text),btrim(name_text),btrim(unit_text),cost,threshold,enabled) returning id into result;
  else
    ...
    update public.products set sku=btrim(sku_text),name=btrim(name_text),unit=btrim(unit_text),cost_centimes=cost,
...
revoke all on function public.save_product(text,text,text,integer,numeric,boolean,uuid,integer) from public,anon;
grant execute on function public.save_product(text,text,text,integer,numeric,boolean,uuid,integer) to authenticated;
...
begin
  ...
  select coalesce(jsonb_agg(r order by r.name,r.id),'[]'::jsonb) into rows from (
    select p.id,p.sku,p.name,p.unit,p.cost_centimes,p.active,p.version,p.reorder_level::text,
